...
class Reserva extends Model
{
    ...
    public function getReserva($param){
        return $this->join('imoveis', 'reservas.id_imovel', '=', 'imoveis.id')
    ...
    public function getResultadoPesquisa($dadosForm, $empresaUserAtual){
        return $this->join('imoveis', 'reservas.id_imovel', '=', 'imoveis.id')
            ->where('reservas.id_empresa', '=', $empresaUserAtual)
            ->where(function($query) use($dadosForm){
                if($dadosForm['id']){
                    $query->where('reservas.id', '=', "{$dadosForm['id']}");
                }
                if($dadosForm['vendedor']){
                    $query->where('reservas.vendedor', '=', "{$dadosForm['vendedor']}");
                }
            })
            ->select('reservas.*', 'imoveis.codigo')
            ->orderBy('reservas.id')
